//! # LendingPool — Core Lending Protocol on Polkadot
//!
//! Equivalent to LendingPool.sol. Deposit wDOT as collateral, borrow USDC.
//! Uses PSP22 cross-contract calls for token interactions.
...
mod lending_pool {
    ...
    const ORACLE_PRECISION: u128 = 100_000_000; // 1e8
    ...
    #[ink(event)]
    pub struct Deposit { #[ink(topic)] user: AccountId, amount: u128 }
    #[ink(event)]
    pub struct Withdraw { #[ink(topic)] user: AccountId, amount: u128 }
    #[ink(event)]
    pub struct Borrow { #[ink(topic)] user: AccountId, amount: u128 }
    #[ink(event)]
    pub struct Repay { #[ink(topic)] user: AccountId, amount: u128 }
    #[ink(event)]
    pub struct Liquidation {
        #[ink(topic)] liquidator: AccountId,
        #[ink(topic)] borrower: AccountId,
        debt_repaid: u128,
        collateral_seized: u128,
    }
    #[ink(event)]
    pub struct LtvUpdated { old_ltv: u128, new_ltv: u128 }
    #[ink(event)]
    pub struct LiquidationParamsUpdated { close_factor: u128, liquidation_incentive: u128 }
    #[ink(event)]
    pub struct OracleUpdated { old_oracle: AccountId, new_oracle: AccountId }
    #[ink(event)]
    pub struct LiquidationThresholdUpdated { old_threshold: u128, new_threshold: u128 }
    ...
    impl LendingPool {
        ...
        pub fn new(
            ...
            assert!(close_factor > 0 && close_factor <= PRECISION, "Invalid close factor");
        ...
        /// Deposit collateral (wDOT). Caller must have approved this contract.
        #[ink(message)]
        pub fn deposit(&mut self, amount: u128) -> Result<(), LiquiFiError> {
            self.ensure_not_paused()?;
            if amount == 0 { return Err(LiquiFiError::ZeroAmount); }

            self.accrue_interest();
            let caller = self.env().caller();

            // Transfer collateral from user to pool
            self.psp22_transfer_from(self.collateral_token, caller, self.env().account_id(), amount)?;
            ...
            self.env().emit_event(Deposit { user: caller, amount });
            Ok(())
        }
        ...
        pub fn withdraw(&mut self, amount: u128) -> Result<(), LiquiFiError> {
            ...
            self.env().emit_event(Withdraw { user: caller, amount });
            Ok(())
        }
        ...
        pub fn borrow(&mut self, amount: u128) -> Result<(), LiquiFiError> {
            ...
            self.env().emit_event(Borrow { user: caller, amount });
            Ok(())
        }
        ...
        pub fn repay(&mut self, amount: u128) -> Result<(), LiquiFiError> {
            ...
            self.psp22_transfer_from(self.debt_token, caller, self.env().account_id(), actual_amount)?;
            self.env().emit_event(Repay { user: caller, amount: actual_amount });
            Ok(())
        }

        /// Liquidate an unhealthy position.
        #[ink(message)]
        pub fn liquidate(&mut self, borrower: AccountId, repay_amount: u128) -> Result<(), LiquiFiError> {
            self.ensure_not_paused()?;
            if repay_amount == 0 { return Err(LiquiFiError::ZeroAmount); }
            let caller = self.env().caller();
            if borrower == caller { return Err(LiquiFiError::SelfLiquidation); }
            ...
            let repay_value_usd = (normalized_repay * debt_price) / (10u128.pow(COLLATERAL_DECIMALS));
            let seize_value_usd = (repay_value_usd * (PRECISION + self.liquidation_incentive)) / PRECISION;
            let mut collateral_seized = (seize_value_usd * (10u128.pow(COLLATERAL_DECIMALS))) / collateral_price;
            ...
            self.psp22_transfer_from(self.debt_token, caller, self.env().account_id(), repay_amount)?;
            self.psp22_transfer(self.collateral_token, caller, collateral_seized)?;

            self.env().emit_event(Liquidation {
                liquidator: caller, borrower, debt_repaid: repay_amount, collateral_seized,
            });
            Ok(())
        }
        ...
        #[ink(message)]
        pub fn get_total_deposits(&self) -> u128 { self.total_deposits }

        #[ink(message)]
        pub fn get_total_borrows(&self) -> u128 { self.total_borrows }
        ...
        #[ink(message)]
        pub fn get_borrower_count(&self) -> u32 { self.borrower_list.len() as u32 }

        #[ink(message)]
        pub fn get_borrower_at(&self, index: u32) -> AccountId { self.borrower_list[index as usize] }

        #[ink(message)]
        pub fn get_borrow_index(&self) -> u128 { self.borrow_index }
        ...
        pub fn get_max_borrow(&self, user: AccountId) -> u128 {
            ...
            if current_usd >= max_borrow_usd { return 0; }
        ...
        #[ink(message)]
        pub fn get_liquidation_threshold(&self) -> u128 { self.liquidation_threshold }
        ...
        #[ink(message)]
        pub fn ltv(&self) -> u128 { self.ltv }

        #[ink(message)]
        pub fn close_factor(&self) -> u128 { self.close_factor }

        #[ink(message)]
        pub fn liquidation_incentive(&self) -> u128 { self.liquidation_incentive }

        #[ink(message)]
        pub fn oracle(&self) -> AccountId { self.oracle }
        ...
        pub fn set_ltv(&mut self, new_ltv: u128) -> Result<(), LiquiFiError> {
            self.ensure_owner()?;
            if new_ltv == 0 || new_ltv > PRECISION { return Err(LiquiFiError::InvalidLtv); }
            let old = self.ltv;
            self.ltv = new_ltv;
            self.env().emit_event(LtvUpdated { old_ltv: old, new_ltv });
            Ok(())
        }
        ...
        pub fn set_liquidation_params(&mut self, cf: u128, li: u128) -> Result<(), LiquiFiError> {
            self.ensure_owner()?;
            if cf == 0 || cf > PRECISION { return Err(LiquiFiError::InvalidCloseFactor); }
            if li > PRECISION { return Err(LiquiFiError::InvalidLiquidationIncentive); }
            self.close_factor = cf;
            self.liquidation_incentive = li;
            self.env().emit_event(LiquidationParamsUpdated { close_factor: cf, liquidation_incentive: li });
            Ok(())
        }
        ...
        pub fn set_oracle(&mut self, new_oracle: AccountId) -> Result<(), LiquiFiError> {
            ...
            self.env().emit_event(OracleUpdated { old_oracle: old, new_oracle });
            Ok(())
        }
        ...
        pub fn set_liquidation_threshold(&mut self, t: u128) -> Result<(), LiquiFiError> {
            self.ensure_owner()?;
            if t < PRECISION { return Err(LiquiFiError::InvalidLtv); }
            let old = self.liquidation_threshold;
            self.liquidation_threshold = t;
            self.env().emit_event(LiquidationThresholdUpdated { old_threshold: old, new_threshold: t });
            Ok(())
        }
        ...
        fn accrue_interest(&mut self) {
            ...
            if self.total_borrows > 0 {
                ...
                let elapsed_sec = (elapsed / 1000) as u128; // ms → sec
        ...
        fn calculate_health_factor(&self, pos: &Position) -> u128 {
            if pos.debt_amount == 0 { return u128::MAX; }
        ...
        fn ensure_owner(&self) -> Result<(), LiquiFiError> {
            if self.env().caller() != self.owner { return Err(LiquiFiError::Unauthorized); }
            Ok(())
        }

        fn ensure_not_paused(&self) -> Result<(), LiquiFiError> {
            if self.paused { return Err(LiquiFiError::Unauthorized); }
            Ok(())
        }

        // ── Internal: Cross-contract calls ──────
        // These use ink!'s built-in cross-contract call builder.

        fn get_oracle_price(&self, asset: AccountId) -> u128 {
            use ink::env::call::{build_call, ExecutionInput, Selector};
            let result = build_call::<ink::env::DefaultEnvironment>()
                .call(self.oracle)
                .exec_input(ExecutionInput::new(Selector::new(ink::selector_bytes!("get_price")))
                    .push_arg(asset))
        ...
        fn query_borrow_rate(&self) -> u128 {
            ...
            build_call::<ink::env::DefaultEnvironment>()
                ...
                .exec_input(ExecutionInput::new(Selector::new(ink::selector_bytes!("get_borrow_rate")))
        ...
        fn psp22_transfer(&self, token: AccountId, to: AccountId, amount: u128) -> Result<(), LiquiFiError> {
            use ink::env::call::{build_call, ExecutionInput, Selector};
            let data: Vec<u8> = Vec::new();
            build_call::<ink::env::DefaultEnvironment>()
                .call(token)
                .exec_input(ExecutionInput::new(Selector::new(ink::selector_bytes!("PSP22::transfer")))
                    .push_arg(to)
                    .push_arg(amount)
                    .push_arg(data))
                .returns::<Result<(), ink::primitives::LangError>>()
                .invoke()
                .map_err(|_| LiquiFiError::Psp22Error(liquifi_traits::Psp22ErrorKind::InsufficientBalance))
        }

        fn psp22_transfer_from(&self, token: AccountId, from: AccountId, to: AccountId, amount: u128) -> Result<(), LiquiFiError> {
            use ink::env::call::{build_call, ExecutionInput, Selector};
            let data: Vec<u8> = Vec::new();
            build_call::<ink::env::DefaultEnvironment>()
                .call(token)
                .exec_input(ExecutionInput::new(Selector::new(ink::selector_bytes!("PSP22::transfer_from")))
                    .push_arg(from)
                    .push_arg(to)
                    .push_arg(amount)
                    .push_arg(data))
                .returns::<Result<(), ink::primitives::LangError>>()
                .invoke()
                .map_err(|_| LiquiFiError::Psp22Error(liquifi_traits::Psp22ErrorKind::InsufficientAllowance))
        }
        ...
        fn psp22_balance_of(&self, token: AccountId, account: AccountId) -> u128 {
            ...
            build_call::<ink::env::DefaultEnvironment>()
                .call(token)
                .exec_input(ExecutionInput::new(Selector::new(ink::selector_bytes!("PSP22::balance_of")))
                    .push_arg(account))
                .returns::<u128>()
                .invoke()
        }
